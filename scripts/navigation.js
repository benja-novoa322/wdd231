...
function renderCourses(filteredCourses) {
    ...
    item.classList.add(course.completed ? "completed" : "not-completed");

    item.innerHTML = `
        <h3>${course.subject} ${course.number} - ${course.title}</h3>
        <p><strong>Credits:</strong> ${course.credits} | 
            <strong>Technologies:</strong> ${course.technology.join(", ")}</p>
    `;

    courseList.appendChild(item);
    });

    creditTotal.textContent = `Courses listed: ${filteredCourses.length} | Total Credits: ${totalCredits}`;
}
